...
1. `src/lib/insets.ts` installs a hidden `env()`-probe element and mirrors its computed padding (+ `visualViewport.height`) into five CSS custom properties on `<html>`: `--sai-top`, `--sai-right`, `--sai-bottom`, `--sai-left`, `--app-h`.
...
| Never use raw `100dvh` for app-frame sizing. Use `var(--app-h, 100dvh)`. | `layout.css`, `app-shell.css` | Raw `100dvh` reads the transient wrong viewport on Android reload. |
...
| `Math.max(current, prev.X)` in `sync()` must stay. | `insets.ts` | Without it, the first post-reload read (which can be 0 on iOS) overwrites the good cached values. |
| `localStorage` writes are gated on `display-mode: standalone || hasInsets > 0`. | `insets.ts` | Prevents caching 0s from a pre-standalone transient state or from browser-mode sessions. |
| The `initInsets()` call must remain at the top of `main.ts`, before any layout-touching code. | `main.ts` | Ensures custom props are kept in sync for the lifetime of the session. |
